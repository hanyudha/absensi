...
<div class="dropdown-menu dropdown-menu-end dropdown-menu-animate-up" aria-labelledby="notificationDropdown">
    ...
    @foreach(auth()->user()->unreadNotifications as $notification)
        <!-- Mengarahkan notifikasi ke halaman cuti dan menandai sebagai dibaca -->
        <a class="dropdown-item" href="{{ route('notifications.markAsRead', $notification->id) }}">
            Status pengajuan cuti kamu {{ $notification->data['status'] }}.
        </a>
    @endforeach
    <div class="dropdown-footer text-center py-2">
        <a href="{{ route('user.markAllRead') }}" class="text-primary">Tandai semua sebagai dibaca</a>
    </div>
</div>
